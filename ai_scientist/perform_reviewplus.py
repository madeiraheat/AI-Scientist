...
def load_paper(pdf_path, num_pages=None, min_size=100):
    try:
        ...
        try:
            doc = pymupdf.open(pdf_path, encoding='utf-8')  # open a document
            if num_pages:
                doc = doc[:num_pages]
            text = ""
...
def perform_improvement(review, coder, papertext):
    improvement_prompt = '''Given this research paper """ {papertext} """,  the following review has been created for it:
"""
{review}
"""

Improve the whole text of the research paper in English using the review and maintaining the same structure. Improve it section by section'''.format(
